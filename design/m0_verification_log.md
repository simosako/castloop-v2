...
**当時のM0判定:** 実際のShow/Episodeメタデータ・feed・固定キー画像を使った途中停止からの収束、M0計画の全停止点、Queue一時停止を伴う例外的な手動中断は未検証だった。OAuth経路とDashboardのFreeプラン確認も未完了だった（後者はM5でFree tierをAPI応答から確認）。CLIでの300 MB超の事前拒否はCLI実装時のファイルサイズ判定とし、超過ファイルをCloudflareにuploadして試すM0項目にはしない。後続M2/M5の製品フロー確認は別ログに記録し、これら模擬試験の履歴を実公開の合格として扱わない。
...
## 2026-09-23: M0アーキテクチャ検証の完了判定

管理者が対象アカウントの**Workers Freeプラン**をDashboardで確認した（本環境のAPI tokenで契約プランを照会した結果ではない）。開発環境はSSHで使用するブラウザのないVPSであり、Wrangler OAuthの対話ログインはここでは必要ない。環境変数のAPI tokenで、検証用リソースの作成・Worker deploy・R2 upload/download・Queueと通知の設定・動作を実測済み。ブラウザのあるクライアントPCで実際に対話運用するときの`wrangler login`は別途確認する。

**判定: M0のアーキテクチャ検証は完了。** 同一ShowのShow/Episode共通受付、停止と同job再試行・DLQ後のブロック、R2 job診断、実R2公開キーの配信とcache purge、300,000,000 bytesのファイル往復は上記の実測記録に従う。製品CLI・本番schema/RSS・immutable mediaを使ったエンドツーエンド公開はM1/M2以降の実装・検証事項。例外的なQueue一時停止・強制中断はMVPの通常の自動回復要件にせず、必要になったときに別途確認する。検証用リソースは後続の検証に使えるよう保持し、使用終了時に片付ける。
