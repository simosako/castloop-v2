...
**残件:** (1) 受付後／marker前のCLI停止、途中停止・古いconsumerの再入を公開snapshotまで含めて安全に回復する方式（所有者の条件付き解放だけでは他の公開objectの遅延書き込みを防げない）、(2) 300 MB超をCLI実装でupload前に拒否すること、(3) OAuthログイン、Free契約のDashboard確認。`max_retries: 2`は**実験設定**であり本番値の決定ではない。検証用bucket・Queue・DLQ・Workerと小さな実験データは続くM0のため保持する。
...
**M0全体は継続:** 実際のShow/Episodeメタデータ・feed・固定キー画像を使った途中停止からの収束、M0計画の全停止点、Queue一時停止を伴う例外的な手動中断は未検証。OAuth経路とDashboardのFreeプラン確認、CLIでの300 MB超の事前拒否も未完了。実験は状態遷移と模擬公開キーの検証であり、実際の公開フローの合格と混同しない。
